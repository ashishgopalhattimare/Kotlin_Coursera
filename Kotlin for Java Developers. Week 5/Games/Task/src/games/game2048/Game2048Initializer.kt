...
import board.GenerateGameBoard
...
object RandomGame2048Initializer: Game2048Initializer<Int> {
    ...
    private fun getRand(len: Int) : Int = Random.nextInt(0, len)
    ...
    override fun nextValue(board: GameBoard<Int?>): Pair<Cell, Int>? {
        ...
        val cells = board.getAllCells().filter { board.get(it) == null }
        var result : Pair<Cell, Int>? = null
        if(cells.isNotEmpty()) {
            result = Pair(cells[getRand(cells.size)],
